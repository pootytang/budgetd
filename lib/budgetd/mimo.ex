defmodule Budgetd.Mimo do
  ...
  defp transaction_query(criteria) do
    ...
    Enum.reduce(criteria, query, fn
      {:budget, budget_id}, query -> from t in query, where: t.budget_id == ^budget_id
      {:order_by, binding}, query -> from t in exclude(query, :order_by), order_by: ^binding
      {:preload, bindings}, query -> preload(query, ^bindings)
      _, query -> query
    end)
  end
  ...
  def summarize_budget_transactions(budget_id) do
    ...
    query
    |> Repo.all()
    |> Enum.reduce(%{}, fn [type, amount], summary -> Map.put(summary, type, amount) end)
  end
end
